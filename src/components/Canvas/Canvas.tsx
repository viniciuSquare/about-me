...
import { DatabaseModel } from './Nodes/DatabaseModel';
...
export function Canvas() {
  ...
  function handleAddNode(nodeType: keyof typeof NODE_TYPES) {
    setNodes((nodes) => {
      return [...nodes, {
  ...
  return (
    <>
      ...
      <Toolbar.Root className="fixed flex bottom-20 left-1/2 -translate-x-1/2 bg-white rounded-2xl shadow-lg border border-zinc-300 px-8 h-20 w-96 overflow-hidden">
        <Toolbar.Button onClick={()=>handleAddNode('square')} className="text-zinc-400">
          <div className="w-32 h-32 bg-violet-500 mt-6 rounded hover:-translate-y-2 transition-transform"></div>
        </Toolbar.Button>
        <Toolbar.Button onClick={()=>handleAddNode('databaseModel')} className="text-red-400">
          <div className="w-32 h-32 bg-red-700 mt-6 rounded hover:-translate-y-2 transition-transform">Database Model</div>
        </Toolbar.Button>
      </Toolbar.Root>
    </>
  );
